...
FPLUS_INCLUDE_DIR = os.path.realpath(f"{THIS_DIR}/../include/fplus")
ALL_IN_ONE_INCLUDE_DIR = os.path.realpath(
    f"{THIS_DIR}/../include_all_in_one/include/fplus"
)
...
def amalgamate_one_file(filename, already_included_files):
    ...
    lines = fread_lines(f"{FPLUS_INCLUDE_DIR}/{filename}")
    for code_line in lines:
        ...
        elif "#pragma once" not in code_line:
...
def write_amalgamation():
    ...
    """
    fwrite_content(f"{ALL_IN_ONE_INCLUDE_DIR}/fplus.hpp", amalgamation_content())
...
def write_fwd_defines(all_function_and_bind_counts):
    ...
    def make_fplus_fwd_define_fn_line(function_and_bind_count):
        return f"fplus_fwd_define_fn_{str(function_and_bind_count.bind_count)}({function_and_bind_count.function_name})"

    def make_fplus_fwd_flip_define_fn_line(function_and_bind_count):
        return f"fplus_fwd_flip_define_fn_1({function_and_bind_count.function_name})"
    ...
    fwrite_content(
        f"{FPLUS_INCLUDE_DIR}/fwd_instances.autogenerated_defines", code
    )
...
def write_curry_defines(all_function_and_bind_counts):
    ...
    def make_curry_define_fn_line(function_and_bind_count):
        return f"fplus_curry_define_fn_{str(function_and_bind_count.bind_count)}({function_and_bind_count.function_name})"
    ...
    fwrite_content(
        f"{FPLUS_INCLUDE_DIR}/curry_instances.autogenerated_defines", code
    )
